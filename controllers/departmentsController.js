...
const createDepartment = async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      'INSERT INTO department (name, description, budget, manager_assignment_id) VALUES (?, ?, ?, ?)',
      [name, description, budget || 0, manager_assignment_id || null]
    );
    res.status(201).json({ department_id: result.insertId, message: 'Department created successfully' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
const updateDepartment = async (req, res) => {
  try {
    ...
    const { name, description, budget, manager_assignment_id } = req.body;
    await pool.execute(
      'UPDATE department SET name = ?, description = ?, budget = ?, manager_assignment_id = ? WHERE department_id = ?',
...
const getDepartmentEmployees = async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      ...
      WHERE ea.department_id = ? AND ea.status = 'ACTIVE' AND e.status = 'ACTIVE'
