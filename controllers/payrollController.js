...
const updatePayrollEntry = async (req, res) => {
  try {
    ...
    try {
      ...
      await connection.execute('DELETE FROM payroll_bonus WHERE payroll_entry_id = ?', [id]);
      if (bonuses && bonuses.length > 0) {
        for (const bonus of bonuses) {
          await connection.execute(
            'INSERT INTO payroll_bonus (payroll_entry_id, bonus_type_id, amount, reason) VALUES (?, ?, ?, ?)',
      ...
      await connection.execute('DELETE FROM payroll_deduction WHERE payroll_entry_id = ?', [id]);
      if (deductions && deductions.length > 0) {
        for (const deduction of deductions) {
          await connection.execute(
            'INSERT INTO payroll_deduction (payroll_entry_id, deduction_type_id, amount, effective_date, reason) VALUES (?, ?, ?, ?, ?)',
  ...
  try {
    ...
    if (month && year) {
      ...
      if (payrollRuns.length > 0) {
        ...
        const [runInfo] = await pool.execute(
      ...
      const [individualRuns] = await pool.execute(
        `SELECT 
                pr.payroll_run_id, 
                pr.status, 
                pr.pay_date, 
    ...
    const result = await Promise.all(filteredRows.map(async (emp) => {
      ...
      if (individualRunsMap && individualRunsMap[emp.employee_id]) {
        ...
        const [runStatus] = await pool.execute(
          `SELECT status, pay_date FROM payroll_run WHERE payroll_run_id = ?`,
  ...
  try {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (currentEntryId) {
          await connection.execute('DELETE FROM payroll_deduction WHERE payroll_entry_id = ?', [currentEntryId]);
...
const payIndividualEmployee = async (req, res) => {
  try {
    ...
    try {
      ...
      const [existingIndividualRuns] = await connection.execute(
      ...
      if (existingIndividualRuns.length > 0) {
        ...
        const [entryForThisRun] = await connection.execute(
        ...
        if (entryForThisRun.length === 0) {
          ...
          if (existingEntry.length > 0) {
            ...
            const [mainEntries] = await connection.execute(
            ...
            if (mainEntries.length > 0) {
              ...
              const [newEntryResult] = await connection.execute(
                `INSERT INTO payroll_entry (payroll_run_id, assignment_id, gross_salary, bonus_amount, net_salary, remarks)
                 VALUES (?, ?, ?, ?, ?, ?)`,
      ...
      try {
        const [verify] = await verifyConnection.execute(
  ...
  try {
    ...
    try {
      ...
      for (const emp of employees) {
        ...
        const [existingPaidRuns] = await connection.execute(
  ...
  try {
    ...
    if (finalStatus === 'PAID') {
      ...
      try {
        ...
        const [finalVerify] = await pool.execute(
          `SELECT status, pay_date FROM payroll_run WHERE payroll_run_id = ?`,
          [id]
        );
...
const getDeductionTypes = async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM deduction_type ORDER BY name');
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
